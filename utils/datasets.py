...
vid_formats = ['mov', 'avi', 'mp4', 'mpg', 'mpeg', 'm4v',
               'wmv', 'mkv', 'webm']  # acceptable video suffixes
...
class LoadStreams:  # multiple IP or RTSP cameras
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        left_cam = cv2.VideoCapture(source_array[0])
        assert left_cam.isOpened(), f'Failed to open {s}'

        right_cam = cv2.VideoCapture(source_array[1])
        assert right_cam.isOpened(), f'Failed to open {s}'
        w = int(left_cam.get(cv2.CAP_PROP_FRAME_WIDTH))
        h = int(left_cam.get(cv2.CAP_PROP_FRAME_HEIGHT))
        self.fps = left_cam.get(cv2.CAP_PROP_FPS) % 100

        _, self.imgs[0] = left_cam.read()  # guarantee first frame
        _, self.imgs[1] = right_cam.read()  # guarantee first frame

        thread_left = Thread(target=self.update, args=(
            [0, left_cam]), daemon=True)
        thread_right = Thread(target=self.update, args=(
            [1, right_cam]), daemon=True)
        thread_left.start()
        thread_right.start()
    ...
    def update(self, index, cap):
        ...
        while cap.isOpened():
            ...
            if n == 4:  # read every 4th frame
                ...
                n = 0
                # self.imgs[1], self.imgs[0] = calibration.undistortRectify(
    ...
    def __next__(self):
        ...
        img0 = self.imgs.copy()
        img0[1], img0[0] = calibration.undistortRectify(
            self.imgs[1], self.imgs[0])
        if cv2.waitKey(1) == ord('q'):  # q to quit
            cv2.destroyAllWindows()
            raise StopIteration
